...
export default function Button({text, icon, path, color, onClick}: ButtonProps) {
    ...
    return (
        <Link href={path} className={`${bg} cursor-pointer px-4 rounded-md h-8 flex justify-evenly items-center gap-2 select-none`}>
            <h1 className='font-bold'>{icon ? `${icon}` : ''}</h1>
