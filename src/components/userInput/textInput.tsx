...
export default function TextInput({
    ...
    const inputClassNameBase =
        'bg-login-600 p-3 pt-6 border-b focus:outline-none focus:ring-0 w-full '
    const colorMix = 'color-mix(in oklab, currentColor 50%, transparent)'

    function select() {
        if (labelRef.current && inputRef.current) {
            labelRef.current.className =
                labelClassNameBase + 'top-2 text-xs text-login'
    ...
    function blur() {
        if (labelRef.current && inputRef.current && document.hasFocus()) {
            if (inputRef.current?.value.length === 0) {
                labelRef.current.className =
                    labelClassNameBase +
                    'top-0 h-full content-center text-red-500'
                labelRef.current.style.color = ''
                inputRef.current.className =
                    inputClassNameBase + 'border-red-500'
                setIsempty(true)
            } else {
                labelRef.current.className =
                    labelClassNameBase + 'top-2 text-xs'
                labelRef.current.style.color =
                    'color-mix(in oklab, currentColor 50%, transparent)'
                inputRef.current.className =
                    inputClassNameBase + 'border-login-200'
            }
        }
    }
        ...
        <div style={{ width }}>
            <div className='relative rounded-t-sm overflow-hidden'>
                ...
                <div
                    ...
                    className={
                        labelClassNameBase + ' top-0 h-full content-center'
                    }
                >
            ...
            <div className='flex'>
                {isEmpty && (
                    <div className='text-red-500 text-xs'>
                        This field is <b className='text-inherit'>requred</b>
                    </div>
                )}
                <div className='text-xs ml-auto text-forground text-login-200'>
                    {inputLen}/50
                </div>
            </div>
        </div>
