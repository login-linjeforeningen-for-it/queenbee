...
export default function List({list, sticky, visible}: ListProps) {
    ...
    return (
        <div className='relativ overflow-visible overflow-y-auto min-h-[calc(100vh-var(--h-navbar)-var(--h-pageInfo))] max-h-[calc(100vh-var(--h-navbar)-var(--h-pageInfo))] noscroll w-fullo max-w-[calc(100vw-var(--w-sidebar))]'>
        ...
        <thead className='bg-normal'>
            <tr>
                {keys.map((key) => {
                    ...
                    return (
                        <th key={key} className={`max-w-[10rem] bg-normal p-2 text-left ${sticky.includes(key) ? 'font-bold sticky left-0 z-10' : 'font-normal'}`}>
                            {value}
                        </th>
                    )
                })}
